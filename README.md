...
### Advanced: Customizing Trusted Proxy Headers

For sites with unusual proxy setups, the `elementtest_trusted_proxy_headers` filter allows you to override which HTTP headers are trusted for visitor IP resolution:

```php
add_filter( 'elementtest_trusted_proxy_headers', function( $headers ) {
    return array( 'HTTP_X_CUSTOM_REAL_IP' );
} );
```
