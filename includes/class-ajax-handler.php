...
class ElementTest_Ajax_Handler {
	...
	public function save_test() {
		...
		if ( isset( $_POST['goals'] ) && is_array( $_POST['goals'] ) ) {
			...
			if ( ! empty( $submitted_goal_ids ) ) {
				$ids_safe     = array_map( 'absint', $submitted_goal_ids );
				$placeholders = implode( ',', array_fill( 0, count( $ids_safe ), '%d' ) );
				$wpdb->query(
					$wpdb->prepare(
						// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- dynamic placeholder count.
	...
	public function proxy_page() {
		...
		// Only allow proxying URLs from this WordPress site via HTTP(S) on expected ports.
		$home_parts   = wp_parse_url( home_url() );
		...
		$target_scheme = isset( $target_parts['scheme'] ) ? strtolower( $target_parts['scheme'] ) : '';
		$target_port   = isset( $target_parts['port'] ) ? (int) $target_parts['port'] : 0;

		if ( $target_host !== $home_host ) {
			wp_die( esc_html__( 'Only pages from this site can be loaded.', 'elementtest-pro' ), 403 );
		}

		// Only allow HTTP/HTTPS schemes to prevent ftp://, file://, etc.
		if ( ! in_array( $target_scheme, array( 'http', 'https' ), true ) ) {
			wp_die( esc_html__( 'Invalid URL scheme.', 'elementtest-pro' ), 403 );
		}

		// Block non-standard ports to prevent internal service probing.
		$allowed_ports = array( 0, 80, 443 );
		if ( isset( $home_parts['port'] ) ) {
			$allowed_ports[] = (int) $home_parts['port'];
		}
		if ( ! in_array( $target_port, $allowed_ports, true ) ) {
			wp_die( esc_html__( 'Invalid port.', 'elementtest-pro' ), 403 );
		}
				...
				foreach ( $test['goals'] as $goal ) {
					...
					$wpdb->insert(
						...
						array(
							...
							'revenue_value'    => isset( $goal['revenue_value'] ) ? max( 0, floatval( $goal['revenue_value'] ) ) : 0.00,
							'created_at'       => $now,
						),
						array( '%d', '%s', '%s', '%s', '%s', '%f', '%s' )
					);
